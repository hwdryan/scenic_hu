...
behavior DriveAvoidingCollisions(target_speed=25, avoidance_threshold=10):
    ...
    interrupt when self.longitudinaldistanceToClosestLeadVehicle() <= avoidance_threshold:
...
behavior FollowRightEdgeBehavior(target_speed = 10, laneToFollow=None, is_oppositeTraffic=False):
    ...
    current_centerline = current_lane.rightEdge
    in_turning_lane = False # assumption that the agent is not instantiated within a connecting lane
    intersection_passed = False
    entering_intersection = False # assumption that the agent is not instantiated within an intersection
    end_lane = None
    ...
    while True:
        ...
        if not entering_intersection and (distance from self.position to nearby_intersection) < TRIGGER_DISTANCE_TO_SLOWDOWN:
            ...
            # assumption: there always will be a maneuver
            if select_maneuver.connectingLane != None:
                current_centerline = concatenateCenterlines([current_centerline, select_maneuver.connectingLane.rightEdge, \
                    select_maneuver.endLane.rightEdge])
            else:
                current_centerline = concatenateCenterlines([current_centerline, select_maneuver.endLane.rightEdge])
...
# self-defined
behavior ReachRightEdgeBehavior(target_speed = 10, laneToFollow=None, is_oppositeTraffic=False):
    """
    Changed based on FollowLaneBehavior.
...
behavior LaneChangeToShoulderBehavior(laneSectionToSwitch, is_oppositeTraffic=False, target_speed=10):
    ...
    current_lane = laneSectionToSwitch.group.shoulder
    traj_centerline = [current_lane.centerline]
    trajectory_centerline = concatenateCenterlines(traj_centerline)

...
# self-defined
behavior WalkwithDirectionBehavior(direction, speed=0.5):
    take SetWalkingDirectionAction(direction), SetWalkingSpeedAction(speed)

behavior WalkBehavior(maxSpeed=1.4):
    take SetWalkAction(True, maxSpeed)

# self-defined
behavior CrossingBehavior_s(reference_actor, direction, min_speed=1, threshold=10, final_speed=None):
    """
    ...
    if isinstance(self, Walks):
        do WalkwithDirectionBehavior(direction, final_speed)
    elif isinstance(self, Steers):

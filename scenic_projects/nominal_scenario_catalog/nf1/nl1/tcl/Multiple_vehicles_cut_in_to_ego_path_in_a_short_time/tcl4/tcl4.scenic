...
# Speed of vehicles
V1_speed = V2_speed = V3_speed = Range(10,12)

# location of vehicles
road_length = 308.69
Ego_loc = Range(1,road_length-125)
destination_loc = Ego_loc + 125
V1_loc = Range(1,Ego_loc)
V2_loc = Range(V1_loc, Ego_loc)
V3_loc = Range(V2_loc, Ego_loc)
overtake_distance = Range(-50,-15)
...
scenario Main():
    setup:
        ...
        # overtake vehicle V3
        overtake_vehicle = new Car following roadDirection from start_spot for V3_loc,  \
                        facing 0.1 deg relative to roadDirection,
                        with color Color(1,0,1), \
                        with blueprint target_vehicle_type, \
                        with rolename "V3", \

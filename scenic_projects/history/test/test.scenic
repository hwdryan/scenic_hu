...
#         wait
#     interrupt when self.SpeedOfEgo() > 0.01:
#         do WalkwithDirectionBehavior(direction=120 deg relative to ped_spot.heading)
...
scenario Main():
    setup:
        ...
        # # Cyclist C1
        # cyslist_spot = new OrientedPoint on roadSec.backwardLanes[0].centerline.start
        # cyclist = new Bicycle following roadDirection from cyslist_spot for 1,  \
        #                 with blueprint 'vehicle.bh.crossbike', \
        #                 # with behavior FollowLaneBehavior(target_speed=1), \
        #                 with rolename "C1"
        
        # # pedestrian
        # curb_middle = new OrientedPoint on roadSec.road.laneGroups[1].curb.start 
